...
Inspect the locally installed API and license first. With Gurobi, `MIPFocus=3` is a documented option for stalled bounds; `MIPFocus=2` targets proving optimality. Compare them only when solver work is appropriate. Avoid primal-switch settings such as `ImproveStartTime` in a dedicated dual run unless specifically motivated. There is no universal best cut aggressiveness, LP method, presolve or thread count.
...
Official references checked 2026-09-16: [Gurobi parameter guidelines](https://docs.gurobi.com/projects/optimizer/en/current/concepts/parameters/guidelines.html), [parameter reference](https://docs.gurobi.com/projects/optimizer/en/current/reference/parameters.html). Check the installed-version documentation before using any parameter.
