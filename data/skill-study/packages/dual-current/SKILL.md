...
When reproducing, comparing, or extending the September 2026 controlled 20-instance campaign, read [experiment-history.md](references/experiment-history.md) and its machine-readable companion [experiment-runs.json](references/experiment-runs.json). Keep the frozen skill used by that campaign distinct from this current operational version; the historical Gurobi campaign did not retroactively use the current COPT-first policy.
...
## Select the solver explicitly

Unless the user or an exact replay protocol specifies a solver, use this order for generic solver-backed work: **COPT, Gurobi, NVIDIA cuOPT, then another compatible solver**. Before committing the main budget, probe candidates in that order. A backend is usable only when its API or executable starts, its license initializes, the actual model loads with the required features, the requested resource limits are accepted, and a short run exposes a documented global dual-bound value with the correct objective-sense and status semantics. Search only environments and hosts already authorized for the task; absence from `PATH` alone does not prove that COPT is unavailable.

Fall back when the install or license is unavailable, the model uses unsupported features, the backend cannot expose the global-bound evidence needed by the route, or loading/starting fails reproducibly. A weak early bound or an ordinary time limit does not make COPT unavailable; another backend may still be tried as a separately logged portfolio route while budget remains. Never switch silently: preserve the partial log and any valid bound, then record the probe result, selected backend, fallback reason, version, host/device, and model hash. User choice overrides this default, and a solver fixed by a strict reproduction protocol remains fixed for that replay.

Express settings in solver-neutral terms, then classify every backend-specific mapping as direct, approximate, unavailable, or an audited model transformation. Do not copy parameter names across solvers or imply semantic equivalence. For cuOPT, verify the installed version's MIP feature coverage, GPU/driver availability, status semantics, and readable global dual bound; a heuristics-only run is not a dual-bound backend. Keep numerical solver bounds separate from independently certified bounds.
