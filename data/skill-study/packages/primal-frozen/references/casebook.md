...
- The population contains 112 instances in the fixed 2026-09-04 open set. The separate `rmine14` directory is historical calibration and is excluded from the denominator.
- The September 8 validation package contains 22 supplied vectors: 18 numerical improvements (including sing17) and 4 first finite solutions. It validates feasibility/objectives, not novelty or global optimality.
...
- The approximately `2.47e-11` relative change for `sing17` is numerical polishing. Three external or externally derived fastxgemm vectors also improve old public records, but the repository explicitly excludes them from project primal contributions.
...
| fhnw-binschedule1 | 55198→55158 | About 1.14 million variables reduced to 773 items; witness constructed by colored matching | Internal machine loads must be checked in addition to endpoint loads |
...
| nj1 | unknown→382.0606208032 | Population-balanced connected partitioning, recombination, kick + descent | Shares the 595-label partition with nj2/3; first find a complete feasible solution, then improve it |
...
| ns1905797 | unknown→13.98258 | Rebalance four assignment blocks, construct CP routes, transfer a complete start, then improve with COPT | 15.03429 → 14.62129 after 5 seconds → 13.98258 after 120 seconds; a 900-second continuation gave no further improvement |
...
- [allcolor58 construction](https://github.com/Huangyc98/MIPLIB_openproblem/blob/b329d3812c5acf51733e0e9f7baabdda7b1008d2/instances/allcolor58/reports/objective-42-certificate.md): fixed capacity reached 55 stores in 53 seconds; flex reached 58 stores in about 40 seconds. These times come from log-file intervals and exclude preliminary research.
...
| Misleading decomposition labels | All 395 small dws012-02 blocks contain linking variables only | Check private variables and coupling rows before using a DEC file to define independent subproblems |
